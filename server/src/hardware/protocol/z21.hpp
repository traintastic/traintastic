...
#ifndef SERVER_HARDWARE_PROTOCOL_Z21_HPP
#define SERVER_HARDWARE_PROTOCOL_Z21_HPP
...
static_assert(sizeof(Message) == 0x04);

struct LanGetSerialNumber : Message
{
  LanGetSerialNumber()
  {
    dataLen = sizeof(LanGetSerialNumber);
    header = LAN_GET_SERIAL_NUMBER;
  }
} __attribute__((packed));
static_assert(sizeof(LanGetSerialNumber) == 0x04);
...
struct z21_lan_x_get_loco_info : z21_lan_x
{
  ...
  z21_lan_x_get_loco_info(uint16_t address, bool longAddress) :
    addressHigh{static_cast<uint8_t>(longAddress ? 0xC0 | (address >> 8) : 0x00)},
    addressLow{static_cast<uint8_t>(address & (longAddress ? 0xFF : 0x7F))}
  {
