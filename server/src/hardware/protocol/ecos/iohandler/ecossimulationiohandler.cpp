      ...
      [this](uint16_t /*channel*/, uint16_t address, bool value)
      {
        if(address >= 1)
        {
          ...
          for(const auto& s88 : m_simulation.s88)
          {
            ...
            if(s88.id == m_simulation.s88.back().id && s88.id < ObjectId::ecosDetectorMin - 1)
            {
              addS88Module();
            }
          }
    ...
    }); itLocomotive != m_simulation.locomotives.end())
  {
    if(request.command == Command::get)
    {
      ...
      for(auto option : request.options)
      {
        ...
        {
          response.append(std::to_string(request.objectId)).append(" funcset[00000000000000000000000000000]\r\n");
        }
