...
TEST_CASE("Board/TrainPathFinder: Direct block to block, variant 6", "[board][train-path-finder]")
{
  EventLoop::reset();
  ...
  REQUIRE(boardWeak.lock()->addTile(1, 1, TileRotate::Deg90, BlockRailTile::classId, false));
  REQUIRE(boardWeak.lock()->addTile(0, 1, TileRotate::Deg180, Curve90RailTile::classId, false));
  REQUIRE(boardWeak.lock()->addTile(0, 0, TileRotate::Deg0, BlockRailTile::classId, false));

  std::weak_ptr<BlockRailTile> block1 = std::dynamic_pointer_cast<BlockRailTile>(boardWeak.lock()->getTile({1, 1}));
  REQUIRE_FALSE(block1.expired());
  std::weak_ptr<BlockRailTile> block2 = std::dynamic_pointer_cast<BlockRailTile>(boardWeak.lock()->getTile({0, 0}));
  REQUIRE_FALSE(block2.expired());
  ...
  // Reserve the path:
  REQUIRE(world->trainPathFinder->reserve(block1.lock(), BlockTrainDirection::TowardsA, block2.lock(), BlockTrainDirection::TowardsB));
...
TEST_CASE("Board/TrainPathFinder: Direct block to block, variant 7", "[board][train-path-finder]")
{
  EventLoop::reset();
  ...
  REQUIRE(boardWeak.lock()->addTile(1, 0, TileRotate::Deg90, BlockRailTile::classId, false));
  REQUIRE(boardWeak.lock()->addTile(0, 0, TileRotate::Deg270, Curve90RailTile::classId, false));
  REQUIRE(boardWeak.lock()->addTile(0, 1, TileRotate::Deg0, BlockRailTile::classId, false));

  std::weak_ptr<BlockRailTile> block1 = std::dynamic_pointer_cast<BlockRailTile>(boardWeak.lock()->getTile({1, 0}));
  REQUIRE_FALSE(block1.expired());
  std::weak_ptr<BlockRailTile> block2 = std::dynamic_pointer_cast<BlockRailTile>(boardWeak.lock()->getTile({0, 1}));
  REQUIRE_FALSE(block2.expired());
...
TEST_CASE("Board/TrainPathFinder: Direct block to block, variant 8", "[board][train-path-finder]")
{
  EventLoop::reset();
